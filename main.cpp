...
int main() {
    constexpr unsigned int NUM_PARTICLES = 100000;
    ...
    analog:{
        ...
        for (int i = 0; i < num_cores; ++i) {
            futures.push_back(std::async(std::launch::async, [&continuous_xsec, &fission_bank, particles_per_core]() {
                ...
                for (unsigned int j = 0; j < particles_per_core; ++j) {
                    ...
                    for (auto neutron : results.neutrons) {
                        thread_results.neutrons.push_back(neutron);
                    }
                }
        ...
        for (auto& f : futures) {
            ...
            fission_bank_new.insert(fission_bank_new.end(), results.neutrons.begin(), results.neutrons.end());
        }
